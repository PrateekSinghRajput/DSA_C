...
int main(){
    ...
    if (head == NULL || head->data >= x) {                //this condition is to check weather list is empty or not
    ...
    while (t1->next != NULL && t1->next->data < x) {        //this condition is to check where new element should be placed

        t1 = t1->next;

    }

    temp->next = t1->next;                                //Insert the new node
